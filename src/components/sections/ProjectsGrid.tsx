...
export const ProjectsGrid = () => {
  ...
  const ProjectCard = ({
    ...
    >
      {/* Architectural Card */}
      <div className="relative aspect-4/5 md:aspect-square overflow-hidden rounded-4xl md:rounded-[2.5rem] bg-obsidian-surface border border-border-dim transition-all duration-1000 group-hover:border-accent/40 group-hover:shadow-[0_0_80px_rgba(0,159,255,0.05)]">
        <motion.img
          src={project.image}
          alt={project.title}
          initial={{
        ...
        <div className="absolute top-6 md:top-10 left-6 md:left-10 z-20 flex flex-col items-start gap-4">
          ...
          <motion.div
            ...
            className="px-4 py-2 border border-border-dim rounded-full bg-obsidian/40 backdrop-blur-xl"
          >
            <span className="text-[8px] font-black uppercase tracking-[0.3em] text-silk-white/60">
              {project.category}
            </span>
          </motion.div>
        </div>
        ...
        <div className="absolute bottom-8 md:bottom-12 left-8 md:left-12 right-8 md:right-12 z-20">
          <h3 className="text-3xl md:text-4xl font-display uppercase tracking-tight leading-none mb-6 text-silk-white">
            {project.title}
          </h3>
          <motion.div
            initial={{ scaleX: 0 }}
      ...
      <div className="mt-12 space-y-6 px-4">
        <p className="text-silk-white/30 text-sm font-light leading-relaxed max-w-sm group-hover:text-silk-white/60 transition-colors duration-700">
          {project.description}
        </p>
      </div>
    </motion.div>
      ...
      <div className="container-luxury space-y-20 md:space-y-32">
        ...
        <div>
          <div className="flex items-center gap-6 mb-16">
            <h2 className="text-xl font-display uppercase tracking-[0.3em] text-silk-white">
              Software Development Projects
            </h2>
            <div className="h-px bg-silk-white/10 grow" />
          </div>
